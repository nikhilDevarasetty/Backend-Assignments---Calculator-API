...
app.post("/add", (req, res) => {
  ...
  if (typeof num1 === "string" || typeof num2 === "string") {
    res.status(500).send({ message: "Invalid data types", sum: result });
  } else if (num1 < -threshold || num2 < -threshold || result < -threshold) {
    res.status(500).send({ message: "Underflow", sum: result });
  } else if (num1 > threshold || num2 > threshold || result > threshold) {
    res.status(500).send({ message: "Overflow", sum: result });
  } else {
    res.send({
